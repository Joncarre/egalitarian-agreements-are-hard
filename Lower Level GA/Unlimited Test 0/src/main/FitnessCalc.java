...
public class FitnessCalc {
    ...
    public boolean setValueAlza(int row, int col) {
    	...
    	if(oldValue > 0) { // Si he votado positivamente la ley
    		double newValue = (int) (oldValue+(oldValue*0.3)); // 50% del valor que le queda por crecer (mientras < 1000)
    		if(newValue < 1000) {
        		M_preferences.get(row).set(col, newValue);
        		result = true;
    		}
    	}else if(oldValue < 0) { // Si he votado negativamente la ley
    		double newValue = (int) (oldValue-(Math.abs(oldValue)*0.3)); // 50% del valor que le queda por crecer (mientras < 0)
    		if(newValue > -1000) {
        		M_preferences.get(row).set(col, newValue);
        		result = true;
    		}
    ...
    public boolean setValueBaja(int row, int col) {		
    	...
    	double oldValue = M_preferences.get(row).get(col);
    	if(oldValue > 0) { // Si he votado positivamente la ley
    		double newValue = (int) (oldValue-(oldValue*0.3)); // 50% del valor que le queda por decrecer (mientras > 0)
    		if(newValue > 0) {
        		M_preferences.get(row).set(col, newValue);
        		result = true;
    		}
    	}else if(oldValue < 0) { // Si he votado negativamente la ley
    		double newValue = (int) (oldValue-((1000+oldValue)*0.3)); // 50% del valor que le queda por decrecer (mientras > -1000)
    		if(newValue < 0) {
        		M_preferences.get(row).set(col, newValue);
        		result = true;
    		}		
