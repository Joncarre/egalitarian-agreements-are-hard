...
public class AGI_Algorithm {
     ...
     * @return
     */
    public static AGI_Population evolvePopulation(AGI_Population popEvolved, int popSize, int generationCount) {
        // Obtener el mejor fitness actual
        AGI_Individual fittest = popEvolved.getFittest();
        ...
        if (inCatastropheMode || generationCount % catastropheInterval == 0) {
            ...
            for(int i = elitismOffset; i < popSize; i++) {
                AGI_Individual indiv = popEvolved.getIndividual(i);
        ...
        for(int i = elitismOffset; i < popEvolved.size(); i++){
            ...
            AGI_Individual indiv2;
            if (Math.random() < determinismFactor) {
                // Torneo de tres candidatos - más selectivo
                AGI_Individual candidate1 = popEvolved.getIndividual(indexes.getRandom());
                AGI_Individual candidate2 = popEvolved.getIndividual(indexes.getRandom());
                AGI_Individual candidate3 = popEvolved.getIndividual(indexes.getRandom());
    ...
    /**
     * Crossover individuals con múltiples tipos de cruce
     * @param indiv1
     * @param indiv2
     * @return
     */
    ...
    /**
     * Mutate an individual con múltiples tipos de mutación
     * @param indiv
     */
     ...
     */
    private static void mutateLarge(AGI_Individual indiv, double temperature) {
        Random random = new Random();
