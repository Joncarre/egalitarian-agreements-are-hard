...
public class AGS_Individual {
    ...
    static int defaultGeneLength = 25;
     ...
     */
    public void formalizeSets() {
    	double auxPositive = 0;
    	double auxNegative = 0;
    	for(int i = 0; i < this.genes.size(); i++) {
    		if(this.genes.get(i) > 0)
    			auxPositive += Math.abs(this.genes.get(i));
    		else
    			auxNegative += Math.abs(this.genes.get(i));
    	}
    	auxPositive = 1000/auxPositive;
    	auxNegative = 1000/auxNegative;
    	for(int j = 0; j < this.genes.size(); j++) {
    		if(this.genes.get(j) > 0)
    			this.genes.set(j, this.getGene(j)*auxPositive);
    		else
    			this.genes.set(j, this.getGene(j)*auxNegative);
    	}
    }
